...
import { getAadhaarData, persistAadhaarVerify } from "@/services/aadhaarService";
import { ROUTES } from "@/constants/ui";

const DetailRow = ({ label, value, isLast }) => (
    <div className={`flex justify-between items-center py-1 ${!isLast ? 'border-b border-slate-50 pb-4' : ''}`}>
        <span className="text-[14px] text-slate-500 font-medium">{label}</span>
        <span className="text-[15px] font-bold text-slate-800">{value || "N/A"}</span>
    </div>
);
...
const CheckInStatusPage = () => {
    ...
    useEffect(() => {
        const processIDVerification = async () => {
            ...
            const storedDigi = JSON.parse(sessionStorage.getItem("digilockerResponse") || "{}");
    ...
    if (isLoading) {
        return (
            <div className="flex flex-col items-center justify-center min-h-screen bg-white">
                ...
                <p className="text-slate-600 font-medium">Fetching Verified Details...</p>
            </div>
        );
    }
    ...
    return (
        <div className="min-h-screen bg-white flex flex-col">
            ...
            <header className="flex items-center px-4 h-16 border-b border-slate-100">
                ...
                <h1 className="flex-1 text-center text-lg font-bold text-slate-800 pr-10">Check-in Status</h1>
            </header>
            ...
            <div className="flex-1 px-4 py-8 overflow-y-auto max-w-md mx-auto w-full">
                <div className="text-center mb-8">
                    ...
                    <p className="text-slate-500 text-sm">Your identity has been securely verified and updated.</p>
                </div>

                <div className="bg-white rounded-3xl border border-slate-100 p-8 shadow-[0_8px_30px_rgb(0,0,0,0.04)]">
                    <div className="flex justify-between items-center mb-10">
                        <h2 className="text-[11px] font-bold text-slate-400 tracking-[0.2em] uppercase">Verification Details</h2>
                        <div className="flex items-center gap-2 py-1.5 px-4 bg-green-50 rounded-full">
                            <div className="w-2 h-2 rounded-full bg-[#10b981] animate-pulse"></div>
                            <span className="text-[12px] font-bold text-[#10b981] tracking-wide">VERIFIED</span>
                        </div>
                    </div>

                    <div className="space-y-6">
                        <DetailRow label="Full Name" value={aadhaarData.full_name || aadhaarData.name} />
                        <DetailRow label="Phone" value={`+${displayData.countryCode} ${displayData.phoneNumber}`} />
                        <DetailRow label="City" value={aadhaarData.split_address?.vtc || aadhaarData.split_address?.dist || "N/A"} />
                        <DetailRow label="State" value={aadhaarData.split_address?.state || "N/A"} isLast />
                    </div>
                </div>
