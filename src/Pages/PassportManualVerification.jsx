...
const PassportManualVerification = () => {
  ...
  // ✅ STORED IMAGES
  const [frontImage, setFrontImage] = useState(null);
  // const [backImage, setBackImage] = useState(null); /* Temprary Commenting Out  */
  ...
  const handleFrontNext = async () => {
    try {
      ...
      const response = await uploadPassport(frontImage);
  ...
  const handleCancelProcessing = async () => {
    ...
    // clear images
    setFrontImage(null);
    // setBackImage(null); /* Temprary Commenting Out  */
  ...
  if (previewStep) {
    ...
    "
        ...
        <div className="mt-6 space-y-3">
          {/* NEXT */}
          <button
            // onClick={isFrontPreview ? handleFrontNext : handleBackSubmit} {/* Temprary Commenting Out  */}
    ...
    <div className="h-dvh bg-[#f5f5f5] px-4 py-5 flex flex-col overflow-hidden">
      ...
      <div className="flex-1 flex flex-col justify-center">
        ...
        >
          ...
          {/* temprary Commenting Out Back Overlay */}
        </div>
      </div>
